...
class Tag extends Base
{
    ...
    public function taglist($params):array
    {
        ...
        $info = $model->where($where)->limit($start, $page_size)->paginate($page_size, false);
     ...
     * @return array
     */
    public function getTagUsers():array
    {
        $users = $this->users;
        $map = [
            'status' => 1,
        ...
        $data = $users->field('id,rules_id,count(id) as num')
            ->where($map)
            ->group('rules_id')->select();
        $data = collection($data)->toArray();
        $return = [];
    ...
    public function move($data):bool
    {
        ...
        $users    = $this->users;
        $userData = $users->field('id,rules_id')->where(['id' => ['in', $ids]])->select();
        $userData = is_object($userData) ? collection($userData)->toArray() : [];
        ...
        try {
            ...
            foreach($userData as $val){
                $rulesArr = explode(',',$val['rules_id']);
                array_splice($rulesArr,array_search($rulesId,$rulesArr),1);
                $rulesId  = $rulesArr ? implode(',',$rulesArr) : '';
                $arr = ['id'=>$val['id'], 'rules_id'=>$rulesId];
                array_push($saveData,$arr);
            }
            $users->saveAll($saveData);
            return true;
        } catch (\Exception $e) {
