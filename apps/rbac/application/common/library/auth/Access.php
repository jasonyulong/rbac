...
class Access extends Drive
{
    ...
    {
        if (!empty($this->userPowers) && isset($this->userPowers['module_auth']['power'])) {
            return $this->userPowers['module_auth']['power'];
        }

        // 从redis获取权限
        $getRedis = $this->redis->handler()->hget(sprintf(Config::get('redis.user_power_all'), $module_id), $user_id);
        if ($getRedis) {
            $moduleAuth = json_decode($getRedis, true);
            $this->userPowers = $moduleAuth;
            if (isset($moduleAuth['module_auth']['power'])) {
                return $moduleAuth['module_auth']['power'];
            }
        }
        ...
        // 岗位可见性权限, 优先查找个人可见权限
        $userAccessAlso = UsersAlso::where(['user_id' => $user_id, 'module_id' => $module_id])->column('keys,values');
        $jobAccessAlso = UsersJobAccessAlso::where(['job_id' => $job_id, 'module_id' => $module_id])->column('keys,values');
        $jobAccessAlso = array_merge($jobAccessAlso, $userAccessAlso);

        // 如果是业务员则获取绑定的帐号可见权限
        if ($this->jobType == 1) {
            $userAccount = OrganizationUserAccount::where(['user_id' => $user_id, 'status' => 1])->column('platform_account_id');
            if (!empty($userAccount)) {
                $account = isset($jobAccessAlso['account']) ? array_merge(explode(',', $jobAccessAlso['account']), array_unique($userAccount)) : $userAccount;
                $jobAccessAlso['account'] = implode(',', $account);
            }
        }
        ...
        $unique = array_unique(array_merge($this->userPowers['menus'], $this->userPowers['nodes']));
        if (!empty($unique)) sort($unique);
        // 系统权限
        ...
        $this->userPowers['module_auth'] = $moduleAuth[$module_id];
        // 将权限写入redis
        $this->setUserPowerRedis($user_id, $module_id, $moduleAuth[$module_id]);
     ...
     * @param $user_id 用户ID
     * @param $module_id 系统ID
     * @param $module_auth 系统权限
     * @return bool
     */
    public function setUserPowerRedis($user_id, $module_id, $module_auth)
    {
        $user_power = $this->redis->handler()->hget(Config::get('redis.user_power'), $user_id);
        $module_power = [];
        if ($user_power) {
            $module_power = json_decode($user_power, true);
        }
        $module_power[$module_id] = $module_auth;
        // 将权限写入Redis
        $this->redis->handler()->hset(Config::get('redis.user_power'), $user_id, json_encode($module_power));

        // 分系统存储一份
        $redisKey = sprintf(Config::get('redis.user_power_all'), $module_id);
        // 将权限所有参数写入redis
        $this->redis->handler()->hset($redisKey, $user_id, json_encode($this->userPowers));
        return true;
    }
