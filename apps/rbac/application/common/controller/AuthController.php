...
class AuthController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        // 系统ID
        $this->system_id = Config::get('app.system_id');
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            if (!$this->auth->isLogin()) {
                ...
                if ($this->request->isAjax()) {
                    $this->error(!empty($this->auth->getError()) ? $this->auth->getError() : __('您的登录信息已失效, 请重新登录'), url(Config::get('url.login'), ['url' => $url]), null, 1);
                }
                $this->redirect(url(Config::get('url.login'), ['url' => $url]));
            }
            // 判断是否需要验证权限
            if (!$this->auth->match($this->noNeedRight)) {
                // 判断控制器和方法判断是否有对应权限
                if (!$this->auth->check($this->system_id, $path)) {
                    Hook::listen('admin_nopermission', $this);
                    $tourl = $path == 'v1/index/index' ? url('v1/login/logout') : url('/');
                    $timeout = IS_DIALOG ? 0 : 30;
                    $this->error(__('您没有权限访问, 请联系管理员'), $tourl, [], $timeout);
                }
            }
        }
        ...
        // 渲染菜单
        $access = new Access();
        $access->setUsers($this->auth->getUser());
        list($menulist, $navlist) = $access->getSidebar($this->system_id, [], $path);
